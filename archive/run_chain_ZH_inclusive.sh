...
SAMPLE_NAME="mumu_ZHbbbb_inclusive_10TeV"
...
if [ -n "$GRIDPACK_DIR" ]; then
    mkdir -p ./grids
    cp "$GRIDPACK_DIR/grid_mumu_ZHbbbb_inclusive"/* ./grids/
    sed -i "/^integrate (bbbb)/i ?rebuild_grids = false\n\$integrate_workspace = \"grids\"" job.sin
fi
...
# --- Move Outputs ---
FINAL_OUT_DIR=$OUTPUT_DIR/job_${JOB_ID}_ZH_inclusive
