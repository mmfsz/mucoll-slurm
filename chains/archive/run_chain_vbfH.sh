...
# Usage:
#   run_chain_vbfH.sh <JOB_ID> <NEVENTS> <OUTPUT_DIR> <MUCOLL_BENCHMARKS_PATH> <SET> [GRIDPACK_DIR]
#
# SET selects the steering variant:
#   1 -> mumu_vbfH_10TeV.sin              (stable H, Pythia8 default decays)
#   2 -> mumu_vbfH_bb_10TeV.sin           (Whizard decays H -> bb)
#   3 -> mumu_vbfH_hybrid_10TeV.sin       (stable H in Whizard, Pythia8 forces H -> bb)
#   4 -> mumu_vbfHbb_inclusive_10TeV.sin  (2->4 inclusive ME, no H resonance)
...
case "$SET" in
  1) SIN=mumu_vbfH_10TeV.sin;             SAMPLE=mumu_vbfH_10TeV;             TAG=set1 ;;
  2) SIN=mumu_vbfH_bb_10TeV.sin;          SAMPLE=mumu_vbfH_bb_10TeV;          TAG=set2 ;;
  3) SIN=mumu_vbfH_hybrid_10TeV.sin;      SAMPLE=mumu_vbfH_hybrid_10TeV;      TAG=set3 ;;
  4) SIN=mumu_vbfHbb_inclusive_10TeV.sin; SAMPLE=mumu_vbfHbb_inclusive_10TeV; TAG=inclusive ;;
  *) echo "Unknown SET=$SET (use 1, 2, 3, or 4)"; exit 1 ;;
esac

echo "Starting job $JOB_ID with $NEVENTS events (vbfH, SET=$SET -> $SIN)"
...
if [ -n "$GRIDPACK_DIR" ]; then
    ...
    sed -i "/^integrate (vbfh)/i ?rebuild_grids = false\n\$integrate_workspace = \"grids\"" job.sin
fi
...
FINAL_OUT_DIR=$OUTPUT_DIR/job_${JOB_ID}_vbfH_${TAG}
